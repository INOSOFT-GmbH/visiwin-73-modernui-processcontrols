@echo off
echo ========================================================
echo  VisiWin7.ProcessControls.Styles.WPF - Build & Publish
echo ========================================================

REM Clean and rebuild the project in Release mode (ensures NuGet package generation)
echo ?? Cleaning previous build outputs...
msbuild VisiWin7.ProcessControls.Styles.WPF.csproj /p:Configuration=Release /p:Platform=AnyCPU /t:Clean

echo ?? Building project in Release mode (auto-generates NuGet package)...
msbuild VisiWin7.ProcessControls.Styles.WPF.csproj /p:Configuration=Release /p:Platform=AnyCPU /t:Rebuild
...
if not exist "bin\Release\*.nupkg" (
    ...
    if exist "VisiWin7.ProcessControls.Styles.WPF.nuspec" (
        ...
        .\nuget.exe pack VisiWin7.ProcessControls.Styles.WPF.csproj -Properties Configuration=Release -OutputDirectory bin\Release
    )
    ...
    if exist "bin\Release\*.nupkg" (
        ...
        echo.
        echo ?? Try running: nuget pack VisiWin7.ProcessControls.Styles.WPF.csproj -Properties Configuration=Release
        echo.
        pause
        exit /b 1
    )
